...
const ReferralList = () => {
    ...
    const paymentMethodPrompt = (withdrawalMethod, userID, userReferralLength) =>{
        setPayLoad({
        ...
        if(withdrawalMethod === "bank"){
            ...
            .then((res)=>{
                ...
                console.log(withdrawalMethodDetails);
                })
            .catch((err)=>{ 
                setIsLoading(false)
                errorMessage(err)
    ...
    const handleBonusPayment = (userReferralCount, userID) =>{
        setIsPaying(true)
        dispatch(bonusPaid(userReferralCount, userID))
        .then((res)=>{
            setIsPaying(false)
                ...
                <div id="myModal" className={`modal ${showBankDetails ? "d-block" : null}`}>
                    <div className="modal-content">
                        ...
                        <div class="details" style={{background: '#cccccc'}}>
                            <span className="h3 mb-5 method text-success">Bank Details {payLoad?.userID}{payLoad?.referralCount}</span>
                    ...
                    <div className="modal-content">
                        ...
                        <div class="details" style={{background: '#cccccc', padding: '', textAlign:'left', margin: '0px'}}>
                            ...
                            <ul style={{marginTop: '20px'}}>
                                ...
                                <button disabled={isPaying} class="btn btn-sm btn-success my-3" onClick={()=>dispatch(bonusPaid(payLoad.referralCount, payLoad.userID))}>{isPaying ? "Loading..." : "Mark Paid"}</button>
